...
public class Matriz {
    ...
    public Matriz eliminarFila(int k) {
        Matriz fila = new Matriz(this.m-1, this.n);
        for (int i = 0; i != k && i < m; i ++){
            for (int j = 0; j < n; j ++){
            fila.datos[i][j] = this.datos[i][j];
            }
        }
        return fila;
    }
    ...
    public Matriz eliminarColumna(int k) {
        Matriz columna = new Matriz(this.m, this.n-1);
        for (int i = 0; i < m; i ++){
            for (int j = 0; j != k && j < n; j ++){
            columna.datos[i][j] = this.datos[i][j];
            }
        }
        return columna;
    }
